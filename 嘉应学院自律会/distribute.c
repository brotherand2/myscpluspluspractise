...
void distrubute()//分配宿管人员
{
	...
	for(i=0;i<26&&isSuccess;i++)//将26名学生随机分配到13个学院进行宿检
	{
		...
		number=0;
		printf("%d、要分配的学生姓名:%s，%s,%s\n",i+1,stu[i].name,stu[i].institude,stu[i].sex);
		while(!isSuccess)//未分配好时，重新随机分配
		{
			    n=rand()%13;
				number++;//产生随机数的次数加1
				if(number>1000)//1000次还不成功，说明最后的一个学生还未分配是因为还未安排好的剩下一个学院就是这个学生的学院，无法分配
				{
					printf("分配失败\n");
					getchar();//暂停一下，可看到错误信息
					break;
				}
